...
| **강설민** | - 설정용 DB 연동 및 영수증/결제 정보 저장 처리<br>- Toss 결제 승인 및 환불 처리 로직 구현<br>- OAuth2 로그인 처리 및 추가 정보 입력 API 구현<br>- JWT 발급 및 재발급 흐름 설계 (RTR 방식)<br>- Redis 기반 DLQ 설계 및 취소 실패 보상 처리 구현<br>- 결제 환불 정책 로직 및 금액 계산 클래스 설계 |
