  ...
  return (
    <section className="text-logo-white bg-background mt-10 flex w-full max-w-[500px] flex-col items-center justify-center rounded-2xl p-8 shadow-lg">
      <form className="flex w-full flex-col gap-6">
        {/* 방 기본 정보 */}
        <InputBlock label="방 제목" placeholder="방 제목을 입력하세요" />
        <TextAreaBlock label="방 설명" placeholder="방 설명을 입력하세요" />

        <div className="grid grid-cols-2 gap-4">
          <SelectBlock label="카테고리" options={["헬스", "요가", "필라테스", "러닝", "기타"]} />
          {/* 공개방 설정 */}
        ...
        {/* 시작일 종료일 커스텀 달력 */}
        <div className="grid grid-cols-2 gap-4">
          <CustomDateInput label="시작일" selectedDate={startDate} setSelectedDate={setStartDate} />
          <CustomDateInput label="종료일" selectedDate={endDate} setSelectedDate={setEndDate} />
        </div>



        <InputBlock label="방 비밀번호 (선택)" type="password" placeholder="비밀번호를 입력하세요 (필요시)" />
        ...
        <div className="mt-6 flex flex-col items-center gap-4">
          ...
          <Link to="/challenge-list">
            <span className="text-sm text-gray-400">챌린지 방 목록으로 이동</span>
          </Link>
        </div>
      </form>
    </section>
